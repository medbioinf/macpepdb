...
use crate::biology::digestion_enzyme::trypsin::{Trypsin, NAME as TRYPSIN_NAME};
use crate::chemistry::amino_acid::{calc_sequence_mass, UNKNOWN};
...
pub fn create_peptides_entities_from_digest<T>(
    digest: &HashMap<String, i16>,
    partition_limits: &Vec<i64>,
    protein_opt: Option<&Protein>,
) -> Result<T>
where
    T: IntoIterator + std::iter::FromIterator<Peptide>,
    T::Item: Into<Peptide>,
{
    digest
        .iter()
