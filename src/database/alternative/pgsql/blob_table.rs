...
impl<'a> BlobTableTrait<'a, Client, PgClient> for BlobTable<'a> {
    ...
    async fn delete(&self, key_prefix: &str) -> Result<()> {
        ...
        let prep_statement = self.client.prepare(&statement).await?;
        let keys: Vec<String> = self
            .client
            .query_raw(&prep_statement, iter::empty::<i8>())
            .await
            .context("Error when selecting keys for deletion")?
            .then(|row| async move {
                let row = row?;
                Ok::<std::string::String, anyhow::Error>(row.get(0))
            })
            .try_collect()
            .await
...
mod tests {
    ...
    async fn test_insert_select_delete() {
        ...
        assert_eq!((res[0].get::<usize, i64>(0),), (0,));
    }
}
